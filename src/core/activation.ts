...
export async function requestChangeEmail(r:{platformId:string;platformUserId:string;newEmail:string;pluginId:string;}):Promise<APIResponse<null>>{if(!r.newEmail)return{success:false,error:"Email required"};try{const res=await fetch(`${API_BASE_URL}/api/figma`,{method:"POST",headers:{"Content-Type":"application/json","X-Action":"request-change-email"},body:JSON.stringify(r)});const d=await res.json();return res.ok?{success:true,data:null,message:d.message}:{success:false,message:d.message}}catch(e){return{success:false,error:String(e)}}}export async function confirmChangeEmail(r:{platformId:string;platformUserId:string;newEmail:string;verificationCode:string;pluginId:string;}):Promise<APIResponse<{email:string;subscriptionTier:string}>>{if(!r.newEmail||!r.verificationCode)return{success:false,error:"Email and code required"};try{const res=await fetch(`${API_BASE_URL}/api/figma`,{method:"POST",headers:{"Content-Type":"application/json","X-Action":"confirm-change-email"},body:JSON.stringify(r)});const d=await res.json();if(!res.ok)return{success:false,message:d.message};return{success:true,data:{email:d.email||r.newEmail,subscriptionTier:d.subscriptionTier||"free"},message:d.message}}catch(e){return{success:false,error:String(e)}}}
